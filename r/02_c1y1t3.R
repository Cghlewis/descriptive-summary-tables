...
Data <- haven::read_sav("c1y1t3_student_2021-06-14.sav")  %>%
...
TocaConProbC1Y1T3 <- fBind(tocacon_des, tocacon_om, tocacon_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaDisrBehC1Y1T3 <- fBind(tocadis_des, tocadis_om, tocadis_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaProSocC1Y1T3 <- fBind(tocapro_des, tocapro_om, tocapro_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaEmotDysC1Y1T3 <- fBind(tocaemo_des, tocaemo_om, tocaemo_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaInternalC1Y1T3 <- fBind(tocaint_des, tocaint_om, tocaint_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaFamProbC1Y1T3 <- fBind(tocafprob_des, tocafprob_om, tocafprob_al) %>%
  rename(C1Y1T3 = "Time")
...
TocaFamInvlvC1Y1T3 <- fBind(tocafinv_des, tocafinv_om, tocafinv_al) %>%
  rename(C1Y1T3 = "Time")
...
CompProSocC1Y1T3 <- fBind(tcomppro_des, tcomppro_om, tcomppro_al) %>%
  rename(C1Y1T3 = "Time")
...
CompEmoRegC1Y1T3 <- fBind(tcompemo_des, tcompemo_om, tcompemo_al) %>%
  rename(C1Y1T3 = "Time")
...
CompAcadC1Y1T3 <- fBind(tcompacad_des, tcompacad_om, tcompacad_al) %>%
  rename(C1Y1T3 = "Time")
...
CompTotC1Y1T3 <- fBind(tcomptot_des, tcomptot_om, tcomptot_al) %>%
  rename(C1Y1T3 = "Time")
...
Data <- haven::read_sav("c1y1t3_teacher_2021-06-14.sav")  %>%
  rename_all(~ (str_replace(., "T._", "")))
...
EfficC1Y1T3 <- fBind(effic_des, effic_om, effic_al) %>%
  rename(C1Y1T3 = "Time")
...
MBIC1Y1T3 <- fBind(mbi_des, mbi_om, mbi_al) %>%
  rename(C1Y1T3 = "Time")
